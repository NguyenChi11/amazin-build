...
function buildpro_about_core_values_customize_register($wp_customize)
{
    if (!class_exists('BuildPro_About_CoreValues_Repeater_Control') && class_exists('WP_Customize_Control')) {
        class BuildPro_About_CoreValues_Repeater_Control extends WP_Customize_Control
        {
            ...
            public function render_content()
            {
                ...
                include get_theme_file_path('inc-components/Appearance-custom-wp/about-us/section-core-values/index.php');
                return;
            }
        }
    }
    ...
    if (isset($wp_customize->selective_refresh)) {
        $wp_customize->selective_refresh->add_partial('buildpro_about_core_values_partial', array(
            ...
            'render_callback' => function () {
                ob_start();
                get_template_part('template-parts/about_us/section-core-values/index');
                return ob_get_clean();
            },
        ));
    }
    add_action('customize_controls_enqueue_scripts', function () {
        wp_enqueue_style(
            ...
            get_theme_file_uri('inc-components/Appearance-custom-wp/about-us/section-core-values/style.css'),
            array(),
            null
        );
        wp_enqueue_script(
            'buildpro-about-core-values-script',
            get_theme_file_uri('inc-components/Appearance-custom-wp/about-us/section-core-values/script.js'),
...
function buildpro_about_core_values_sync_customizer_to_meta($wp_customize_manager)
{
    ...
    if ($enabled_val === null) {
        $enabled_val = get_theme_mod('buildpro_about_core_values_enabled', 1);
    }
    if ($title_val === null) {
    ...
    if ($items_val === null) {
        $items_val = get_theme_mod('buildpro_about_core_values_items', array());
    }
    $enabled = absint($enabled_val);
    $title = is_string($title_val) ? $title_val : '';
    $desc = is_string($desc_val) ? $desc_val : '';
    $items = buildpro_about_core_values_sanitize_items($items_val);
    $page_id = 0;
    if ($wp_customize_manager instanceof WP_Customize_Manager) {
    ...
    if ($page_id) {
        update_post_meta($page_id, 'buildpro_about_core_values_enabled', $enabled);
        update_post_meta($page_id, 'buildpro_about_core_values_title', $title);
        update_post_meta($page_id, 'buildpro_about_core_values_description', $desc);
        update_post_meta($page_id, 'buildpro_about_core_values_items', $items);
    }
}
