<?php
function buildpro_material_gallery_add_meta_box($post_type, $post)
{
	if ($post_type !== 'material') {
		return;
	}
	add_meta_box('buildpro_material_tab_gallery', 'Gallery', 'buildpro_material_gallery_render_meta_box', 'material', 'normal', 'default');
}
add_action('add_meta_boxes', 'buildpro_material_gallery_add_meta_box', 10, 2);

function buildpro_material_gallery_render_meta_box($post)
{
	wp_enqueue_media();
	$gallery_raw = get_post_meta($post->ID, 'material_gallery_ids', true);
	...
	echo '<script>
	(function(){
		var addBtn = document.getElementById("buildpro_material_add_gallery");
		var clearBtn = document.getElementById("buildpro_material_clear_gallery");
		var input = document.getElementById("buildpro_material_gallery_ids");
		var wrap = document.getElementById("buildpro_material_gallery_wrap");
		var frame;
