...
function buildpro_material_banner_render_meta_box($post)
{
	...
	echo '<p class="buildpro-post-field"><label>Banner Image</label><input type="hidden" id="buildpro_material_banner_id" name="material_banner_id" value="' . esc_attr($banner_id) . '"> <button type="button" class="button" id="buildpro_material_select_banner">Select photo</button> <button type="button" class="button" id="buildpro_material_remove_banner">Remove photo</button></p>';
	echo '<div class="image-banner-preview" id="buildpro_material_banner_preview">' . ($thumb ? '<img src="' . esc_url($thumb) . '">' : '<span style="color:#888">No photo selected</span>') . '</div>';
	echo '</div>';
	echo '<script>
	(function(){
		...
		if(selectBtn){
			selectBtn.addEventListener("click", function(e){
				e.preventDefault();
				if(!frame){ frame = wp.media({ title: "Select photo", button: { text: "Use photo" }, multiple: false, library: { type: "image" } }); }
		...
		if(removeBtn){
			removeBtn.addEventListener("click", function(e){
				e.preventDefault();
				input.value = "";
				preview.innerHTML = "<span style=\\"color:#888\\">No photo selected</span>";
			});
		}
	})();
	</script>';
