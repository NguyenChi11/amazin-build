...
function buildpro_post_section_render_meta_box($post)
{
    ...
    wp_enqueue_style('buildpro-post-admin', get_theme_file_uri('inc-components/custom-wp/home/post/style.css'), array(), null);
    wp_enqueue_script('buildpro-post-admin', get_theme_file_uri('inc-components/custom-wp/home/post/script.js'), array(), null, true);
    wp_add_inline_script('buildpro-post-admin', 'window.buildproPostState=' . wp_json_encode(array('enabled' => $enabled)) . ';', 'before');
    include get_theme_file_path('inc-components/custom-wp/home/post/index.php');
}
